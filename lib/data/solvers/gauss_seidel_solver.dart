...
class GaussSeidelSolver implements ISolverRepository {
  const GaussSeidelSolver();
  ...
  SolverResult solve(LinearSystem system, {Fraction? tolerance}) {
    ...
    for (var i = 0; i < n; i++) {
      if (a[i][i].abs() <= _epsilon) {
        return const SolverFailure(
          'No se puede aplicar Gauss-Seidel: existe un cero en la diagonal principal.',
        );
      }
    }
    ...
    steps.add(
      SolutionStep(
        title: 'Fórmulas iterativas de Gauss-Seidel',
        description: _gaussSeidelFormulasDescription(a, b, variableNames),
        matrixState: _buildAugmented(a, b),
      ),
    );
    ...
    for (var iter = 1; iter <= maxIterations; iter++) {
      final prev = List<double>.from(current);
      for (var i = 0; i < n; i++) {
        var sum = 0.0;
        for (var j = 0; j < n; j++) {
          if (j == i) {
        ...
        current[i] = (b[i] - sum) / a[i][i];

        if (!current[i].isFinite) {
          return const SolverFailure(
            'El sistema no converge con el método de Gauss-Seidel',
          );
        }
      }

      final error = _maxError(prev, current);
      if (!error.isFinite) {
        return const SolverFailure(
          'El sistema no converge con el método de Gauss-Seidel',
        );
  ...
  String _gaussSeidelFormulasDescription(
    List<List<double>> a,
    List<double> b,
    List<String> variableNames,
  ) {
    final lines = <String>[
      'Cada despeje usa valores nuevos para j<i y previos para j>i.',
    ];
    for (var i = 0; i < a.length; i++) {
      final leftTerms = <String>[];
      final rightTerms = <String>[];
      for (var j = 0; j < a[i].length; j++) {
        if (j == i) {
          continue;
        }
        if (j < i) {
          leftTerms.add('${_numberText(a[i][j])}${variableNames[j]}^{(k+1)}');
        } else {
          rightTerms.add('${_numberText(a[i][j])}${variableNames[j]}^{(k)}');
        }
      }

      final parts = <String>[];
      if (leftTerms.isNotEmpty) {
        parts.addAll(leftTerms);
      }
      if (rightTerms.isNotEmpty) {
        parts.addAll(rightTerms);
      }

      final rhs = parts.isEmpty
          ? _numberText(b[i])
          : '${_numberText(b[i])} - ${parts.join(' - ')}';
